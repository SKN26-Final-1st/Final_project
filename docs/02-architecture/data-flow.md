...
5. Django는 `backend/api/urls.py`에 등록된 view로 요청을 보냅니다. 구현은 `backend/api/views/` 패키지에 도메인별로 분리되어 있습니다.
...
근거: `backend/api/views/chat_endpoints.py`, `backend/common/chat_graph.py`, `backend/common/chat_agent.py`
