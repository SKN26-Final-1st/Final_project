...
- 프론트엔드: React, Vite, TypeScript, Ant Design 기반의 운영 화면입니다. 주요 진입점은 `frontend/src/main.tsx`, 라우팅과 전역 화면 상태는 `frontend/src/App.tsx`에 있습니다.
...
- AI 분석: 지원서 분석은 `backend/common/report.py`, 문서/HR 채팅은 `backend/common/chat_graph.py`와 `backend/common/chat_agent.py`가 담당합니다.
...
3. JD에 연결된 지원서를 등록하거나 목 데이터로 확인합니다.
4. 지원서 분석 요청이 백엔드의 `resume/analize/` 엔드포인트로 전달됩니다.
