...
현재 관리자 화면은 백엔드 `AuthKey` API와 직접 연결되어 있지 않고, `AdminData`와 화면 내부 계산을 사용합니다. 근거: `frontend/src/api/adapters.ts`, `frontend/src/pages/AdminPage.tsx`
...
저장 버튼은 `apiClient.saveUserProfile()`을 호출합니다. 실제 API 모드에서는 `/api/account/modify/`로 전달됩니다.
...
mock 모드에서는 `authDefaultsApiResponse`를 초기값으로 사용합니다. 근거: `frontend/src/data/apiMockData.ts`, `frontend/src/api/backendClient.ts`
